...
public interface AssessmentRepository extends JpaRepository<Assessment, Long> {
    ...
    @Query("SELECT a FROM Assessment a WHERE a.student.institution.id = :institutionId AND (a.evaluatedBy.userAccount.username = :username OR a.student.id IN (SELECT DISTINCT s.id FROM Student s JOIN s.enrolledCourses c WHERE c.teacher.userAccount.username = :username))")
